...
module.exports = class UserController{
    static async register(req, res){
        ...
        // Cria o usuário e em seguida cria o token de sessão dele
        try {
    ...
    static async login(req, res) {
        ...
        // Verifica se a senha dada pelo usuário bate com o hash de senha do mesmo
        const checkPassword = await bcrypt.compare(password, user.password)

        if(!checkPassword){
            res.status(422).json({message: 'A senha não corresponde com o email!'})
            return
        }
